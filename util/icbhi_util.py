...
def get_individual_cycles_torchaudio_beats(args, recording_annotations, data_folder, filename, sample_rate, n_cls):
    """
    SCL paper..
    used to split each individual sound file into separate sound clips containing one respiratory cycle each
    output: [(audio_chunk:np.array, label:int), (...)]
...
def get_score(hits, counts, pflag=False):
    ...
    for i in range(len(hits)):
        tp = hits[i]  # True Positives for class i
        fp = sum(hits) - tp  # False Positives for class i
        fn = counts[i] - hits[i]  # False Negatives for class i
    ...
    precision = sum([metrics[2] for metrics in class_metrics]) / len(class_metrics)
    recall = sum([metrics[2] for metrics in class_metrics]) / len(class_metrics)
    f1 = sum([metrics[2] for metrics in class_metrics]) / len(class_metrics)
    
    if pflag:
        # print("************* Metrics ******************")
        print("S_p: {}, S_e: {}, Score: {}".format(sp, se, sc))
        print("Precision (Normal): {}, Recall (Normal): {}, F1 Score (Normal): {}".format(precision, recall, f1))
